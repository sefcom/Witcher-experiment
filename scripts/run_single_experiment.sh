...
if [[ ${DO_DOCKER_RUN} = true ]]; then
    docker kill $docker_container_name; sleep 1;

    if docker run -id --rm --privileged --name ${docker_container_name} ${DEBUG_VOLUME} -v ${BASE_DIR}:${BASE_DIR} -v $app_role_dpath/coverages:/tmp/coverages ${docker_image_name}; then
        printf "[\033[32mWitcher\033[0m] Issued docker run \n"
    else
        exit 2
    fi
    
    if docker exec -it ${docker_container_name} bash -c "chown www-data:wc /tmp/coverages; sudo chmod 777 /tmp/coverages"; then
...
if [[ ${DO_BURPPLUS} = true ]]; then
    ...
    for f in $(find ${target_app_dpath} -maxdepth 2 -name request_data.json); do
...
echo docker exec -it -w "$(pwd)" -u wc ${docker_container_name} bash -i -c 'p'

if docker exec -it -w "$(pwd)" -u wc ${docker_container_name} bash -i -c 'p --testver WICHR'; then

...
cd /tmp;
#export LD_PRELOAD=/wclibs/libcgiwrapper.so
...
LD_PRELOAD=/wclibs/libcgiwrapper.so AFL_META_INFO_ID=${PORT} STRICT=3 SHOW_WITCH=1 PORT=${PORT} /p/Witcher/java/jdksrc/build/linux-x86_64-normal-server-release/jdk/bin/java -XX:+UseSerialGC -XX:+WitcherInstrumentation -jar webgoat-server-8.1.0.jar --server.address=0.0.0.0 --server.port=${PORT}

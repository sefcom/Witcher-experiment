...
if [[ ${DO_BURPPLUS} = true ]]; then
    ...
    echo find $SCRIPT_DIR/.. -name request_data.json
    for f in $(find $SCRIPT_DIR/.. -name request_data.json); do
       python3 $SCRIPT_DIR/../../burp-buff.py --burpproxy 127.0.0.1:$BURP_PORT --target-ip $ipaddr $f
       res=$?
       if [[ $res -ne 0 ]]; then
...
if [[ ${DO_CRAWL} = true ]]; then
    ...
    timeout --signal KILL 4h execute request_crawler http://$ipaddr$base_url_path "$(pwd)" &
    pid=$!
...
if docker exec -it -w "$(pwd)" -u wc $cve-$plus bash -i -c 'p --testver EXWICHR'; then
